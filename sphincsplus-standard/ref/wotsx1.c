...
void wots_gen_leafx1(unsigned char *dest,
    ...
    // FILE *file = fopen("sk_values.txt", "a");
...
// fprintf(file, "-----------leaf index %d ------------------\n",leaf_idx);
    for (i = 0, buffer = pk_buffer; i < SPX_WOTS_LEN; i++, buffer += SPX_N) {
        ...
        // fprintf(file,"length number %d\n ",i+1);
        // fprintbstr(file, "C0 = ", buffer, SPX_N);
        ...
        for (k=0;; k++) {
            ...
            // fprintf(file, "C_%d",k+1);
            // fprintbstr(file, " = ", buffer, SPX_N);
        }
    }

// fprintf(file, "-----------leaf index %d end------------------\n\n\n\n",leaf_idx);
// fclose(file); 
